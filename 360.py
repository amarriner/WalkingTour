...
for i in range(int(step_index),int(step_index) + 1):
   ...
   for x in range(0, 360):
      step_url = 'http://maps.googleapis.com/maps/api/streetview?size=480x480&sensor=false&location=' + \
                    ...
                    '&heading=' + str(x) + '&key=' + keys.google_api_key

      print step_url
      urllib.urlretrieve(step_url, pwd + 'frames/step' + str(x) + '.jpg')
...
for x in range(0, 360):
   print 'Processing ' + pwd + 'frames/step' + str(x) + '.jpg'
   step_image = Image.open(pwd + 'frames/step' + str(x) + '.jpg')
   ...
   combo.save(pwd + 'frames/combo' + increment + '.jpg', 'jpeg')
